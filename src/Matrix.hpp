...
public:
    ...
    void setRow(std::size_t rowIndex, Vector<NumericalType>& vec) {
        if (rowIndex > nRows_) {
            std::string errorMessage = "[HydroForest::Matrix::setRow] `rowIndex` exceeds matrix size:\n";
        ...
        if (vec.size() != nCols_) {
            std::string errorMessage = "[HydroForest::Matrix::setRow] Size of `vec` is not the same as number of columns in `this`:\n";
    ...
    void setColumn(std::size_t colIndex, Vector<NumericalType>& vec) {
        if (colIndex > nCols_) {
            std::string errorMessage = "[HydroForest::Matrix::setColumn] `colIndex` exceeds matrix size:\n";
        ...
        if (vec.size() != nRows_) {
            std::string errorMessage = "[HydroForest::Matrix::setColumn] Size of `vec` is not the same as number of rows in `this`:\n";
            errorMessage += "\tvec.size() = " + std::to_string(vec.size()) + "\n";
            errorMessage += "\tnRows = " + std::to_string(nRows_) + "\n";
            std::cerr << errorMessage << std::endl;
            throw std::invalid_argument(errorMessage);
    ...
    Matrix<NumericalType> getFromIndexSet(Vector<int> I, Vector<int> J) {
        if (I.size() > nRows_) {
            std::string errorMessage = "[HydroForest::Matrix::getFromIndexSet] Size of index set `I` is greater than number of rows in `this`:\n";
        ...
        if (J.size() > nCols_) {
            std::string errorMessage = "[HydroForest::Matrix::getFromIndexSet] Size of index set `J` is greater than number of columns in `this`:\n";
        ...
        for (auto i = 0; i < I.size(); i++) {
            for (auto j = 0; j < J.size(); j++) {
                if (I[i] > nRows_ || I[i] < 0) {
                    std::string errorMessage = "[HydroForest::Matrix::getFromIndexSet] Index in `I` is out of range:\n";
                    errorMessage += "\ti = " + std::to_string(i) + "\n";
                    errorMessage += "\tI[i] = " + std::to_string(I[i]) + "\n";
                    errorMessage += "\tnRows = " + std::to_string(nRows_) + "\n";
                }
                if (J[j] > nCols_ || J[j] < 0) {
                    std::string errorMessage = "[HydroForest::Matrix::getFromIndexSet] Index in `J` is out of range:\n";
    ...
    Matrix<NumericalType> blockPermute(Vector<int> I, Vector<int> J, Vector<int> R, Vector<int> C) {
        ...
        if (rowCheck != nRows_) {
            std::string errorMessage = "[HydroForest::Matrix::blockPermute] Rows in `R` do not add up to number of rows in `this`:\n";
        ...
        if (colCheck != nCols_) {
            std::string errorMessage = "[HydroForest::Matrix::blockPermute] Rows in `C` do not add up to number of rows in `this`:\n";
    ...
    Matrix<NumericalType> getBlock(std::size_t rowIndex, std::size_t colIndex, std::size_t rowLength, std::size_t colLength) {
        if (rowIndex + rowLength > nRows_) {
            std::string errorMessage = "[HydroForest::Matrix::getBlock] Row size exceeds matrix size:\n";
            errorMessage += "\trowIndex = " + std::to_string(rowIndex) + "\n";
        ...
        if (colIndex + colLength > nCols_) {
            std::string errorMessage = "[HydroForest::Matrix::getBlock] Column size exceeds matrix size:\n";
            errorMessage += "\tcolIndex = " + std::to_string(colIndex) + "\n";
    ...
    void setBlock(std::size_t rowIndex, std::size_t colIndex, Matrix<NumericalType>& mat) {
        if (rowIndex + mat.nRows() > nRows_) {
            std::string errorMessage = "[HydroForest::Matrix::setBlock] Row size exceeds matrix size:\n";
            errorMessage += "\trowIndex = " + std::to_string(rowIndex) + "\n";
        ...
        if (colIndex + mat.nCols() > nCols_) {
            std::string errorMessage = "[HydroForest::Matrix::setBlock] Column size exceeds matrix size:\n";
            errorMessage += "\tcolIndex = " + std::to_string(colIndex) + "\n";
    ...
    Matrix<NumericalType>& operator+=(Matrix<NumericalType>& rhs) {
        if (rhs.nRows() != nRows_) {
            std::string errorMessage = "[HydroForest::Matrix::operator+=] Number of rows in `rhs` is not equal to number of rows in `this`:";
        ...
        if (rhs.nCols() != nCols_) {
            std::string errorMessage = "[HydroForest::Matrix::operator+=] Number of columns in `rhs` is not equal to number of columns in `this`:";
...
static Matrix<double> operator*(Matrix<double>& A, Matrix<double>& B) {
    if (A.nCols() != B.nRows()) {
        std::string errorMessage = "[HydroForest::Matrix::operator*=] Invalid matrix dimensions, `A` must have same number of columnes as rows in `B`:\n";
...
static Vector<double> solve(Matrix<double>& A, Vector<double>& b) {
    if (A.nRows() != A.nCols()) {
        std::string errorMessage = "[HydroForest::Matrix::solve] Matrix `A` is not square:\n";
    ...
    if (A.nRows() != b.size()) {
        std::string errorMessage = "[HydroForest::Matrix::solve] Matrix `A` and vector `b` are not the correct size:\n";
        errorMessage += "\tA.nRows() = " + std::to_string(A.nRows()) + "\n";
    ...
    // Check output
    if (INFO_) {
        std::cerr << "[HydroForest::Matrix::solve] Fortran call to `dgesv_` returned non-zero flag of: " << INFO_ << std::endl;
    }
...
static Matrix<double> solve(Matrix<double>& A, Matrix<double>& B) {
    ...
    if (A.nRows() != B.nRows()) {
        std::string errorMessage = "[HydroForest::Matrix::solve] Matrix `A` and `B` are not the correct size:\n";
        errorMessage += "\tA.nRows() = " + std::to_string(A.nRows()) + "\n";
...
NumericalType matrixInfNorm(Matrix<NumericalType>& A, Matrix<NumericalType>& B) {
    if (A.nRows() != B.nRows()) {
        std::string errorMessage = "[HydroForest::Matrix::matrixInfNorm] Number of rows in `A` is not equal to number of rows in `B`:";
        errorMessage += "\tA.nRows = " + std::to_string(A.nRows()) + "\n";
        errorMessage += "\tB.nRows = " + std::to_string(B.nRows()) + "\n";
        std::cerr << errorMessage << std::endl;
        throw std::invalid_argument(errorMessage);
    ...
    if (A.nCols() != B.nCols()) {
        std::string errorMessage = "[HydroForest::Matrix::matrixInfNorm] Number of columns in `A` is not equal to number of columns in `B`:";
        errorMessage += "\tA.nCols = " + std::to_string(A.nCols()) + "\n";
