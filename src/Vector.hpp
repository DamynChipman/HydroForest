...
public:
    ...
    Vector<NumericalType> getSegment(std::size_t startIndex, std::size_t length) {
        if (startIndex + length > size_) {
            std::string errorMessage = "[HydroForest::Vector::getSegment] Index mismatch. `startIndex` + `length` is greater than size of vector:\n";
    ...
    void setSegment(std::size_t startIndex, const Vector<NumericalType>& vec) {
        if (startIndex + vec.size() > size_) {
            std::string errorMessage = "[HydroForest::Vector::setSegment] Index mismatch. `startIndex` + `vec.size()` is greater than size of host vector:\n";
            errorMessage += "\tstartIndex = " + std::to_string(startIndex) + "\n";
    ...
    Vector<NumericalType> blockPermute(Vector<int> I, Vector<int> S) {
        ...
        if (sizeCheck != size_) {
            std::string errorMessage = "[HydroForest::Vector::blockPermute] Sizes in `S` do not add upt to the size of `this`:\n";
